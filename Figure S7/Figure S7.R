setwd("C:/Users/admin/Desktop/data and scripts/Figure S7/")
data <- read.table("Figure S7.txt", header = TRUE, row.names = 1)
library(ggplot2)
library(pheatmap)
data <- as.matrix(data)
write.table(data, "data.tab", sep="\t", quote=FALSE)
data <- read.table("data.tab")
zhushi<-read.table("Figure S7zhushi.txt",header=T,sep="\t",row.names=1)
...
zhushi2<-read.table("Figure S7zhushi2.txt",header=T,sep="\t",row.names=1)
