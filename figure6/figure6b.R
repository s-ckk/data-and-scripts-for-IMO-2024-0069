setwd("C:/Users/admin/Desktop/data and scripts/figure6/")
data <- read.delim("figure6b.txt", row.names = 1, sep = '\t', stringsAsFactors = FALSE, check.names = FALSE)
library(vegan)
...
custom_colors <- c("#1f77b4", "#ff7f0e", "#2ca02c", "#d62728", "#9467bd", "black")

ggplot(data = merged, aes(NMDS1, NMDS2)) +
  geom_point(size = 2, aes(color = factor(cluster), shape = factor(cluster))) +
  stat_ellipse(aes(fill = factor(cluster)), geom = 'polygon', level = 0.95, alpha = 0.1, show.legend = FALSE) +
